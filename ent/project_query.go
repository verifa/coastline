...
func (pq *ProjectQuery) QueryRequests() *RequestQuery {
	query := &RequestQuery{config: pq.config}
...
func (pq *ProjectQuery) Clone() *ProjectQuery {
	...
	return &ProjectQuery{
		...
		withRequests: pq.withRequests.Clone(),
...
func (pq *ProjectQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Project, error) {
	...
	if query := pq.withRequests; query != nil {
		if err := pq.loadRequests(ctx, query, nodes,
...
func (pq *ProjectQuery) loadRequests(ctx context.Context, query *RequestQuery, nodes []*Project, init func(*Project), assign func(*Project, *Request)) error {
	...
	query.Where(predicate.Request(func(s *sql.Selector) {
		s.Where(sql.InValues(project.RequestsColumn, fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.request_project
		if fk == nil {
			return fmt.Errorf(`foreign-key "request_project" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "request_project" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
